...
ControllerAll = false
local ListSudos ={Sudo_Id,1804133252}  
...
local N = (Redis:get(Saidi.."Name:Bot") or "كلير")
...
local StatusMember = LuaTele.getChatMember(bana.id,Saidi).status.luatele
...
LuaTele.sendText(msg_chat_id,msg_id,'*𖥔 عدد احصائيات البوت الكامله \n⩹━━━━𖥕Saidi𖥕━━━━⩺\n𖥔 عدد المجموعات : '..(Redis:scard(Saidi..'ChekBotAdd') or 0)..'\n𖥔 عدد المشتركين : '..(Redis:scard(Saidi..'Num:User:Pv') or 0)..'*',"md",true)  
end
...
return LuaTele.sendText(msg_chat_id,msg_id,'𖥔 عدد الاعضاء قليل لا يمكن تفعيل المجموعه  يجب ان يكوم اكثر من :'..Redis:get(Saidi..'Num:Add:Bot'),"md",true)  
end
...
if Redis:sismember(Saidi.."BanGroup:Group"..msg_chat_id,bana.id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(bana.id,"𖥔 تم حظره من الجروب مسبقا ").Reply,"md",true)  
else
Redis:sadd(Saidi.."BanGroup:Group"..msg_chat_id,bana.id) 
LuaTele.setChatMemberStatus(msg.chat_id,bana.id,'banned',0)
...
if not Redis:sismember(Saidi.."BanGroup:Group"..msg_chat_id,bana.id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(bana.id,"𖥔 تم الغاء حظره من الجروب مسبقا ").Reply,"md",true)  
else
Redis:srem(Saidi.."BanGroup:Group"..msg_chat_id,bana.id) 
LuaTele.setChatMemberStatus(msg.chat_id,bana.id,'restricted',{1,1,1,1,1,1,1,1,1})
...
LuaTele.sendText(msg_chat_id,msg_id,'𖥔 عدد رسائلك هنا *~ '..(Redis:get(Saidi..'Num:Message:User'..msg.chat_id..':'..msg.sender.user_id) or 1)..'*',"md",true)  
elseif text == 'سحكاتي' or text == 'تعديلاتي' then
LuaTele.sendText(msg_chat_id,msg_id,'𖥔 عدد التعديلات هنا *~ '..(Redis:get(Saidi..'Num:Message:Edit'..msg.chat_id..msg.sender.user_id) or 0)..'*',"md",true)  
elseif text == 'جهاتي' then
LuaTele.sendText(msg_chat_id,msg_id,'𖥔 عدد جهاتك المضافه هنا *~ '..(Redis:get(Saidi.."Num:Add:Memp"..msg.chat_id..":"..msg.sender.user_id) or 0)..'*',"md",true)  
...
local Namebot = (Redis:get(Saidi.."Name:Bot") or "كلير")
local BotName = {
...
if text == (Redis:get(Saidi.."Name:Bot") or "كلير") then
local photo = LuaTele.getUserProfilePhotos(Saidi)
local ban = LuaTele.getUser(Saidi)
local Namebot = (Redis:get(Saidi.."Name:Bot") or "كلير")
local BotName = {
...
local CmdStart = '*\n𖥔 أهلآ بك في بوت '..(Redis:get(Saidi.."Name:Bot") or "كلير")..
