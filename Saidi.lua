...
return LuaTele.sendText(msg_chat_id,msg_id,'* ✧ اليك قائمه الالعاب أبدا الان العاب *',"md",false, false, false, false, reply_markup)
end
if text == 'تحديث' then
if not msg.ControllerBot then 
return LuaTele.sendText(msg_chat_id,msg_id,'\n* ✧ هاذا الامر يخص〘 '..Controller_Num(1)..' 〙* ',"md",true)  
...
local UserId = Text:match('(%d+)/Noooo')
if tonumber(IdUser) == tonumber(UserId) then
Redis:del(Saidi.."Send:Bc:Grops"..ChatId)
LuaTele.editMessageText(ChatId,Msg_id,"* ✧ تم الغاء الاذاعه للمجموعات *","md",true) 
end
end
